...
# FOR LOOP START
condition_list = ['arrow', 'parent_CJ', 'parent_JW', 'interest_CJ', 'interest_JW']
for ind, cond in enumerate(condition_list):
    ...
    filename = os.sep.join([main_dir, 'design', 'task-' + cond + '_counterbalance_ver-01_block-02.csv'])
...
#                df.loc[ixndex, 'key_conversion'] = key_map[ptbKey[0][0]]
        kbClock.reset()
    file_savename = os.sep.join([main_dir, 'data', 'sub-01', 'sub-01_task-' + cond + '_ver-01_block-02_beh.csv'])
...
message = visual.TextStim(disp, text='This is the end of session2. Please wait for instructions from the experimenter')
message.draw()  # Automatically draw every frame
disp.flip()
