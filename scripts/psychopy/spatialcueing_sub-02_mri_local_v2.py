...
target_positions = {'left': (int(disp.size[0] * -0.20), 0), 'right':( int(disp.size[0] * 0.20), 0)}
# 7) keyboard mapp
...
# FOR LOOP START
condition_list = ['interest_CJ', 'parent_CJ', 'arrow', 'parent_JW', 'interest_JW']
for ind, cond in enumerate(condition_list):
    ...
    filename = os.sep.join([main_dir, 'design', 'task-' + cond + '_counterbalance_ver-02_block-02.csv'])
    ...
    for index, row in cb_parameters.iterrows():
        ...
        kbClock.reset()
    file_savename = os.sep.join([main_dir, 'data', 'sub-02', 'sub-02_task-' + cond + '_ver-02_block-02_beh.csv'])
...
message = visual.TextStim(disp, text='This is the end of session 2. Please wait for instructions from the experimenter')
message.draw()  # Automatically draw every frame
disp.flip()
